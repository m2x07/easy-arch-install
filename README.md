...
## Setting up the host
First we set the hostname in `/etc/hostname` file. A host name is like a nickname for your computer. You may take a moment to choose one for yourself, and remember it. \
Once you've chosen, open up the file and write your hostname into it.
```bash
vim /etc/hostname
```
> [!TIP]
...
You will be promped to choose which package to install couple of times. Simply go with default option. To do so, just hit ENTER each time you are asked.


Fix sreen's color issue by removing a few packages (won't break your installtion):
```bash
...
Skip this step if you already installed KDE Plasma.
## DWM
...
> [!TIP]
> You may now go ahead and __apply the patches__ you want from [suckless.org](https://suckless.org) \
